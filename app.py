...
def generate_response(user_text):
    if not GOOGLE_API_KEY:
        return "Google API key not configured. Add it to environment or Streamlit secrets."
    try:
...
def get_coping_tip_for(mood):
    tips = {
        ...
        "Angry": "Take a short walk or count to 10, then try to describe what made you angry in one sentence.",
...
with right:
    ...
    st.write("- Use the chat for immediate support.\n- Log mood daily to build a streak.\n- Add helpline numbers in the README before release.")

st.markdown("---")
st.header("📂 Mood History")
if not load_moods().empty:
    hist = load_moods()
    st.dataframe(hist.sort_values("date_time", ascending=False).reset_index(drop=True))
...
st.markdown("---")
st.header("⚠️ Crisis & Safety (Important)")
st.write("This prototype is NOT a substitute for professional help. Add local helpline numbers for your country/region in the README and HELPLINES config before public release.")
